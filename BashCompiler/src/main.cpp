	...
	prefs.prefsMap=
		{
			...
			//{prefs.LFSTK_hashFromKey("shell"),{TYPESTRING,"shell","Use specific shell for some external commands ( default is /bin/bash )\n\nBe aware some shells dont't support certian bash builtins eg there is no 'select' in dash","/bin/bash",false,0}},
...
//	if(prefs.LFSTK_getString("shell").length()>0)
//		shellPath=prefs.LFSTK_getString("shell").c_str();
