	...
	prefs.prefsMap=
		{
			...
			{prefs.LFSTK_hashFromKey("shell"),{TYPESTRING,"shell","Use specific shell ( default is /bin/sh )","/bin/sh",false,0}},
	...
	if(prefs.LFSTK_getBool("syntax-check")==true)
		{
			...
			exitstatus=QProcess::execute(shellPath,QStringList()<<"-c"<<checkstr);
